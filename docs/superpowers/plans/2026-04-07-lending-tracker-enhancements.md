...
- [x] **Step 1: Add `dueAt` nullable integer column to loans table**

- [x] **Step 2: Add schema migration** (add ALTER TABLE for `dueAt` column)

- [x] **Step 3: Update `Loan` Freezed entity** — add `dueAt` (int?), add `isOverdue` getter (`dueAt != null && returnedAt == null && DateTime.now().millisecondsSinceEpoch > dueAt!`)

- [x] **Step 4: Run build_runner**
- [x] **Step 5: Update loan mapper** to handle `dueAt`
- [x] **Step 6: Write tests** — `isOverdue` returns true/false correctly
- [x] **Step 7: Run tests**
- [x] **Step 8: Commit**
...
- [x] **Step 1: Add `watchOverdueLoans()` to DAO** — active loans where `dueAt < now` and `returnedAt` is null

- [x] **Step 2: Add `watchOverdueLoans()` to `ILoanRepository`**

- [x] **Step 3: Implement in `LoanRepositoryImpl`**

- [x] **Step 4: Write DAO tests** in `test/unit/data/dao/loans_dao_overdue_test.dart`
...
- [x] **Step 1: Add `update` method to `IBorrowerRepository`**

- [x] **Step 2: Implement in `BorrowerRepositoryImpl`**

- [x] **Step 3: Add `updateBorrower` to `ManageBorrowersUseCase`**
...
- [x] **Step 1: Add `overdueLoansProvider`** — streams overdue loans from repository

- [x] **Step 2: Add `loansForBorrowerProvider`** — family provider by borrower ID

- [x] **Step 3: Add `overdueCountProvider`** — derived count for badges

- [x] **Step 4: Write provider tests** in `test/unit/presentation/providers/lending_provider_test.dart`
...
- [x] **Step 1: Add `flutter_local_notifications` dependency**

- [x] **Step 2: Create `NotificationService`** — initialises channels for Android/iOS/macOS, exposes `scheduleNotification(id, title, body, scheduledDate)` and `cancelNotification(id)`

- [x] **Step 3: Create `ScheduleOverdueNotificationUseCase`** — when a loan is created with a `dueAt`, schedules a notification for that date. When a loan is returned, cancels the notification.

- [x] **Step 4: Create `notificationServiceProvider`**

- [x] **Step 5: Wire into `LendItemUseCase`** — schedule notification after creating loan with due date

- [x] **Step 6: Wire into `ReturnItemUseCase`** — cancel notification on return

- [x] **Step 7: Write tests** in `test/unit/domain/schedule_overdue_notification_test.dart`
...
- [x] **Step 1: Add optional due date picker to lend dialog**
...
- [x] **Step 2: Pass `dueAt` through to `LendItemUseCase`**

- [x] **Step 3: Show due date on active loan display** — with overdue badge if past due

- [x] **Step 4: Add "Extend Due Date" action** on active loans — opens date picker to update `dueAt`
...
- [x] **Step 1: Create `OverdueBadge` widget** — small chip with error colour background, "OVERDUE" text, and days overdue count. Reusable across item detail, collection list, and borrower screens.

- [x] **Step 2: Add overdue indicators to collection list items** — show badge on items that have an overdue active loan
...
- [x] **Step 1: Create `BorrowersScreen`** — list of all borrowers with:
...
- [x] **Step 2: Add route** — `/borrowers` or nested under settings

- [x] **Step 3: Add navigation entry** — Settings screen tile or sidebar entry

- [x] **Step 4: Write widget tests** in `test/presentation/screens/borrowers/borrowers_screen_test.dart`
...
- [x] **Step 1: Create `BorrowerDetailScreen`** showing:
...
- [x] **Step 2: Add route** — `/borrowers/:id`

- [x] **Step 3: Write widget tests** in `test/presentation/screens/borrowers/borrower_detail_screen_test.dart`
...
- [x] **Step 1: Run full test suite** — `flutter test`
- [x] **Step 2: Run static analysis** — `flutter analyze`
- [x] **Step 3: Manual smoke test** — lend item with due date, verify notification scheduled, return item, verify notification cancelled, check borrowers screen, check overdue badges
