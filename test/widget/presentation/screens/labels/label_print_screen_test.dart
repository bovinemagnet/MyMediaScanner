...
import '../../../../_fakes/fake_printing_platform.dart';
...
void main() {
  ...
  testWidgets(
      'tapping Preview/print after selecting a location calls '
      'Printing.layoutPdf with the generated bytes',
      (tester) async {
    final fake = installFakePrintingPlatform();

    when(() => locationRepo.watchAll()).thenAnswer(
      (_) => Stream.value([_location(id: 'loc1', name: 'Living Room')]),
    );
    when(() => mediaRepo.watchByStatus(any())).thenAnswer(
      (_) => Stream.value([]),
    );
    ...
    // Select the only location so onPreview becomes non-null.
    await tester.tap(find.byType(CheckboxListTile).first);
    await tester.pumpAndSettle();
    expect(find.text('1 selected'), findsOneWidget);

    // Drive the print action.
    await tester.tap(find.text('Preview / print'));
    await tester.pumpAndSettle();

    // Printing.layoutPdf was invoked with the document name the screen
    // hard-codes; the onLayout callback was awaited (so the PDF
    // generator actually ran) and produced non-empty bytes.
    expect(fake.layoutPdfCalls, hasLength(1));
    final call = fake.layoutPdfCalls.single;
    expect(call.name, 'mymediascanner-labels');
    expect(call.bytes, isNotEmpty);
    // PDF magic header — every well-formed PDF starts with "%PDF".
    expect(
      String.fromCharCodes(call.bytes.take(4)),
      '%PDF',
      reason: 'layoutPdf should receive a real PDF byte stream',
    );
  });
}
